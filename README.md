...
本项目支持在Windows、macOS和Linux系统上开发，自动适配不同平台的命令和环境设置：

**平台特性**：
- **Windows**: 自动设置UTF-8字符编码（chcp 65001），保证中文显示正常
- **macOS**: 使用原生UTF-8编码，无需额外设置
- **Linux**: 使用原生UTF-8编码，支持标准开发环境

**技术实现**：
- 内置跨平台脚本运行器 (`cross-platform-runner.js`)
...
- `npm run dev-debug` - 调试模式（带开发者工具）
- `npm run dev-no-reload` - 无热重载开发模式
...
✅ Node.js版本: Node.js v22.11.0 版本符合要求
✅ npm版本: npm 10.9.0 版本符合要求
✅ Git版本控制: git version 2.39.5 (Apple Git-154)
✅ Electron依赖: Electron ^27.0.0 已配置
✅ Node模块: node_modules目录存在
✅ Electron构建器: electron-builder可用
✅ 构建工具: Xcode Command Line Tools已安装
✅ 必需文件: 所有必需文件都存在
...
如果遇到问题，可以使用内置的错误处理工具获取详细的诊断信息和解决方案：
