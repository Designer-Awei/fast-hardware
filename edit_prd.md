...
### 阶段四：集成测试
- [ ] **元件库系统集成** - 实现元件预览与绘制页的双向集成
  - [ ] 预览卡片导入功能：点击预览按钮进入详情窗口，新增"编辑"按钮
  - [ ] 编辑跳转逻辑：读取对应元件JSON，自动填充参数并渲染到元件绘制页
  - [ ] 保存增强：修改完成后保存时选择存储路径（标准/自定义），校验重复并处理
  - [ ] JSON格式验证：自动校验生成的JSON格式完整性和正确性
- [ ] **边界情况测试**
  - [ ] 元件名称重复处理（覆盖/重命名）
  - [ ] JSON格式错误处理
  - [ ] 文件读写权限测试
