...
###### 第四周：项目导入导出 ⭐⭐⭐
- [ ] **项目导入功能**: 点击"加载项目"按钮，打开项目文件夹选择对话框
- [ ] **项目验证**: 检查项目文件夹是否包含必需的`circuit_config.json`和`components/`文件夹
- [ ] **元件索引读取**: 根据`circuit_config.json`中的`componentFile`字段正确读取项目内的元件JSON文件
- [ ] **画布状态重建**: 解析circuit_config.json，恢复元件位置、朝向、连线关系等
- [ ] **项目导出功能**: 点击"保存项目"按钮，显示项目信息输入对话框
- [ ] **项目信息录入**: 填写项目名称（必需）、项目描述（可选）
- [ ] **项目结构创建**: 自动创建components文件夹、circuit_config.json和固件代码文件
- [ ] **存储位置**: 保存到设置页指定的项目存储路径
- [ ] **简化项目结构**: 单个电路原型项目仅包含components/、circuit_config.json和.ino文件
- [ ] **元数据整合**: 项目基本信息（创建时间、修改时间、名称、描述）统一记录在circuit_config.json中
...
**阶段二完成度**: 90% (设置系统已完成，数据同步和用户体验优化待完善)
